...
function drawReducedPolygon02(areaPolygon, selectedPosition){
	...
	for(i = 0; i < turf.coordAll(areaPolygon).length - 1; i++){
		...
		//console.log(turf.coordAll(line));
		...
		reducedPolygon[i] = turf.getCoords(offsetLine);
	}

	//console.log(reducedPolygon);
	cleanLayerById('route');
	map.addSource('route', {
		'type': 'geojson',
		'data': {
			'type': 'Feature',
	...
	newPolygon = [];
	console.log(reducedPolygon.length);
	// Calcular intersecção do novo polígono interno
